...
const Contraption = struct {
    ...
    pub fn reset(self: *Self) void {
        for (self.cells, 0..) |row, i| {
            for (row, 0..) |_, j| {
                self.energized[i][j] = false;
                self.traversed[i][j] = Traversed.init();
            }
        }
    }

    pub fn countEnergizedAll(self: *Self) u32 {
        var most_energized: u32 = 0;
        for (self.cells, 0..) |row, i| {
            const row_i: i32 = @as(i32, @intCast(i));
            const end: i32 = @as(i32, @intCast(row.len)) - 1;
            const beam_right = Beam{ .coord = Coordinate{ .x = 0, .y = row_i }, .direction = Direction.right };
            const beam_left = Beam{ .coord = Coordinate{ .x = end, .y = row_i }, .direction = Direction.left };
            self.traverse(beam_right);
            const cur_energized_right = self.countEnergized();
            if (cur_energized_right > most_energized) {
                most_energized = cur_energized_right;
            }
            self.reset();
            self.traverse(beam_left);
            const cur_energized_left = self.countEnergized();
            if (cur_energized_left > most_energized) {
                most_energized = cur_energized_left;
            }
            self.reset();
        }

        for (self.cells[0], 0..) |_, i| {
            const col_i: i32 = @as(i32, @intCast(i));
            const end: i32 = @as(i32, @intCast(self.cells.len));
            const beam_down = Beam{ .coord = Coordinate{ .x = col_i, .y = 0 }, .direction = Direction.down };
            const beam_up = Beam{ .coord = Coordinate{ .x = col_i, .y = end }, .direction = Direction.up };
            self.traverse(beam_up);
            const cur_energized_up = self.countEnergized();
            if (cur_energized_up > most_energized) {
                most_energized = cur_energized_up;
            }
            self.reset();
            self.traverse(beam_down);
            const cur_energized_down = self.countEnergized();
            if (cur_energized_down > most_energized) {
                most_energized = cur_energized_down;
            }
            self.reset();
        }
...
pub fn main() !void {
    ...
    const sum = contraption.countEnergizedAll();
